...
module tb_trace_slot1_store;
    ...
    initial begin
        ...
        for (cyc = 0; cyc < 15; cyc = cyc + 1) begin
            @(posedge clk);
            $display("cyc=%0d d0=%b d1=%b ld=%b st=%b sq0v=%b sq0addr=%h mwr=%b maddr=%h mdata=%h hlt=%b",
                cyc,
                dut.dispatch0_en,
                dut.dispatch1_en,
                dut.lsq_inst.ld_disp_en,
                dut.lsq_inst.st_disp_en,
                dut.lsq_inst.sq_v[0],
